...
The APP CLUSTER belongs to _neither_ layer cleanly — it's the substrate
...
- **operator** _(today)_ — a k8s controller that reconciles `App` CRs into
  `Deployment`/`Service`/`Ingress` (+TLS). **No database**; idempotent; mechanical.
- **control plane** _(planned)_ — a **Postgres-backed** service holding the product's
...
because today business state lives _only_ in the single app node's etcd (local disk, no
HA) and Apps are imperative (not in git), so a node _rebuild_ loses it. Until the control
...
- A third thing is **neither**: bex deploying a _user's_ repo into a sandbox — that's
