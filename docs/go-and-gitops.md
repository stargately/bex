...
**The pile of imperative setup** the MVP required (and which is _not yet reproducible_):
...
right call to prove the flow fast (zero deps), but the _target_ is a Kubernetes control
...
- Deps stay as they are (we don't rewrite the Python OpenSandbox server or the charts —
  we _consume_ them). "Mostly Go" = **our** code is Go; we talk to deps over their APIs/CRDs.
...
by **Postgres** as the product's _source of truth_ (tenants / apps / domains + business
...
bex's **product** _is_ a deploy-from-git system (webhook → build → deploy). Per-tenant app
...
- **Self-serve tenant creation**: dynamic, via the bex control plane (it's part of the
  product). Pick per how self-serve tenanting needs to be; likely _bootstrap via GitOps,
  scale tenants dynamically_.
